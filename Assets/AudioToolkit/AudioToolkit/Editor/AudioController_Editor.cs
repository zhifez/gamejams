                ...
                if ( curCat != null )
                {
                    ...
                    if ( itemFoldout = EditorGUILayout.Foldout( itemFoldout, "Audio Item Settings", foldoutStyle ) )
                    {
                        ...
                        if ( curItem != null )
                        {
                            ...
                            EditBool( ref curItem.DestroyOnLoad, "Stop When Scene Loads", "If disabled, this audio item will continue playing if the current scene is unloaded. Note that if the audio is parented to another game object it will nevertheless stop playing if the parent object gets destroyed during the scene change." );
